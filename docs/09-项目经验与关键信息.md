...
| **登录浮层** | 未登录时页面叠加登录浮层，须终止流程并引导用户登录 | **已处理** | 见 §4 与 selectors.LOGIN_OVERLAY_TEXTS、engine._check_login_required |
...
| 现象/错误 | 可能原因 | 应对建议 |
|-----------|----------|----------|
| **登录浮层** | 未登录或 session 失效时，抖音会在前台页面上叠加登录浮层（扫码登录/验证码登录等） | 引擎在每次 `search_videos`、`fetch_comments`、`fetch_user_info` 前及加载目标页后做**登录必检**：① 用 `GET user/settings` 校验 session；② 检测页面是否含「扫码登录」「验证码登录」等 `LOGIN_OVERLAY_TEXTS`。任一未通过则触发 danger、终止流程，由 M3/前端提示用户先完成登录（如调用 `login()` 或打开登录浏览器）。 |
| （其他随开发遇到时补充） | | |
...
- **2026-02-27**：§4 风控：增加「登录浮层」现象及处理说明；引擎在 search/fetch_comments/fetch_user_info 中接入 `_check_login_required()`（session 校验 + 登录浮层文案检测），未登录或检测到浮层时触发 danger 并终止流程。
